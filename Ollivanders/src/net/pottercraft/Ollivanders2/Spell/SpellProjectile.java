...
public abstract class SpellProjectile {
	...
	/**
	 * Default constructor should only be used for fake instances of the spell
	 * such as when initializing the book text.
	 */
	 ...
	 *
	 * @param radius
	 *            - radius within which to get entities
			...
			if (e instanceof LivingEntity) {
				if (((LivingEntity) e).getEyeLocation().distance(location) < radius || ((e instanceof EnderDragon || e instanceof Giant) && ((LivingEntity) e).getEyeLocation().distance(location) < (radius + 5))) {
					if (!e.equals(player)) {
						...
						if (lifeTicks > 1) {
							// getLogger().info(((LivingEntity)
							// e).getEyeLocation().distance(location));
							close.add(e);
						}
					}
				}
	...
	/**
	 * Provides the uses modifier that takes into account spell uses and wand
	 * type. Returns 10.0 if the uses are 100 and the right wand is held.
	 *
	 ...
	 *
	 * @param loc
	 *            - The Location that is the center of the block list
	 * @param radius
	 *            - The radius of the block list
	...
	/**
	 * Reverts any changes made to blocks if the effects are temporary. Changed
	 * blocks are in this.changed
	 */
	public void revert() {
