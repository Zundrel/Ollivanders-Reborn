...
public final class PROTEGO_MAXIMA extends Charms {
	...
	public void checkEffect() {
		...
		if (getBlock().getType() != Material.AIR && getBlock().getType() != Material.FIRE && getBlock().getType() != Material.WATER && getBlock().getType() != Material.STATIONARY_WATER) {
			...
			net.pottercraft.Ollivanders2.StationarySpell.PROTEGO_MAXIMA max = new net.pottercraft.Ollivanders2.StationarySpell.PROTEGO_MAXIMA(player, location, StationarySpells.PROTEGO_MAXIMA, 5, duration, damage);
			max.flair(10);
			p.addStationary(max);
			kill();
		}
