...
public abstract class BlockTransfigurationSuper extends SpellProjectile implements O2Spell {
	...
	/**
	 * Whether this transfiguration permanent or not. Usually for Charms it is
	 * false and for Transfiguration it is true.
	 */
	protected boolean permanent = true;
	/**
	 * How many blocks out from the target are affects. Usually for permanent
	 * spells this is 1.
	 */
	...
	/**
	 * A blacklist of Material types that will not be affected by this spell.
	 * Only used if the whitelist is empty.
	 */
	...
	/**
	 * The description text for this spell in spell books. Required or spell
	...
	public void checkEffect() {
		...
		if (!isTransfigured) {
			...
			if (target != null) {
				// if the server is running WorldGuard, make sure the player has
				// permissions
	...
	/**
	 * Transfigure the target block or blocks. Will not change the block if it
	 * is on the materialBlacklist list or if the target block is already the
	 * transfigure type.
	 */
	...
	protected boolean canTransfigure(Block b) {
		...
		if (m == transfigureType) // do not change if this block is already the
									// target type
		{
			canChange = false;
		} else if (b.getState() instanceof Entity) // do not change if this
													// block is an Entity
		{
			canChange = false;
		} else if (materialWhitelist.isEmpty()) // do not change if this block
												// is in the blacklist
		{
		...
		} else if (!materialWhitelist.contains(m)) // do not change if this
													// block is in the whitelist
		{
			canChange = false;
		}
